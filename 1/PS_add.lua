...
..'<Data>'.."\n"
..'    <CompositeFBType Name="T_PS_Add" Comment="Для создания дополнительных ПС" UUID="ACB75AA74AAFE17F2A6C428C8C84D74C">'.."\n"
..'        <InterfaceList>'.."\n"
..'            <InputVars>'.."\n"
...
--if Fild_Serch == 'PS' then 
   ...
   local f_struct = io.open('T_PS_Add.txt',"w+");
   f_struct:write(Template_Alarm_Start)
	
	cur=0
	for i=1, #IO_Struct  do
	  if string.find(IO_Struct[i].Type, 'PS') ~= nil then
    cur = cur+1
	  x_FB = cur*250
	...
	for i=1, #IO_Struct  do
	  ...
	  cur = cur+1
		...
		NameAlg = APS_Struct_Name[i].NameAPS 
