...
dofile('APS.lua') 
local f_out = io.open('alarm_msg_ppu_as.csv','w+'); -- open file
-- hat
...
for i=1,#APS_Struct do
  ...
  if Type == 'АОсс' or Type == 'АОбс' or Type == 'НОсс' or Type == 'НОбс' then 
    f_out:write(tostring(ii)..';Digital;algGVL.GPA_APS.'..ustName..';=;TRUE;;;;;Error;АС:'..Comment..';t#0s;;;'..'\n')
    ii=ii+1
  end
end
