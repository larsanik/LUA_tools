...
local f_out = io.open('GPA_AI_CallAll.txt','w+'); -- открытие фа
f_out:write('//***************************************'..'\n')
...
for i=1,#AI_Struct do
    ...
    f_out:write(' VGSig.fnAI_init(setStruct => algGVL.GPA_AI_Setting.'..AlgName..', min_PV := '..min_PV..', max_PV :=  '..max_PV..', minADC := '..minADC..', maxADC := '..maxADC..', loLim := '..loLim..', hiLim := '..hiLim..', loBrk := '..loBrk..', hiBrk := '..hiBrk..', maxROC:= 0.0, recoveryTime:= 5.0, repairTime :=repTime, tau:= 1.0, id := '..i..', corrADC := 0.0);'..'\n')
    end
end
...
for i=1,#AI_Struct do
  if AnsiToUtf8(AI_Struct[i].Node)==selectedNode or selectedNode=='все' then  
    f_out:write('//['..AnsiToUtf8(AI_Struct[i].Node)..'] ['..AnsiToUtf8(AI_Struct[i].Address)..'] ['..AnsiToUtf8(AI_Struct[i].AdrCh)..'] '..AnsiToUtf8(AI_Struct[i].Comment)..'\n')
    AlgName=AnsiToUtf8(AI_Struct[i].Name)
    f_out:write('VGSig.fnAI_Processing(IN := algGVL.GPA_AI_DRV.'..AlgName..', set := algGVL.GPA_AI_Setting.'..AlgName..', btn := algGVL.GPA_AI_FromHMI.'..AlgName..', out := algGVL.GPA_AI_ToHMI.'..AlgName..', my := algGVL.GPA_AI_Internal.'..AlgName..', nOk :=  algGVL.alg.AI_FLT);'..'\n')
  end
end
f_out:write('//Конец сгенерированного кода'..'\n')
f_out:flush() 
